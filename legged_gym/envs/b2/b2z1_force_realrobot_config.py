...
class B2Z1ForceRealRobotRoughCfg( LeggedRobotCfg ):
    ...
    class commands:
        curriculum = False
        max_curriculum = 1.
        num_commands = 12 # default: lin_vel_x, lin_vel_y, ang_vel_yaw (in heading mode ang_vel_yaw is recomputed from heading error)
    ...
    class rewards( LeggedRobotCfg.rewards ):
        ...
        class scales: # ( ManipLocoCfg.rewards.scales ):
            ...
            tracking_lin_vel = 3. # 1.5  # track x轴方向速度
            # tracking_lin_vel_x_l1 = 0.
            # tracking_lin_vel_x_exp = 0
            tracking_ang_vel = 1.5 # just for yaw # track 旋转速度
            ...
            # energy = -1e-6
            energy_square = -2e-8
            energy_square_arm = -1e-7
            torques = -1.e-6 # -1e-5 # 惩罚力量大小
            ...
            ang_vel_xy = -0.1 # -0.1 # 惩罚过快的转弯速度
            dof_acc = -2.5e-7 #-2.5e-7 # -0.1 # 惩罚过快的joint 加速度
            dof_vel = -5.e-4
            dof_acc_arm = -5.0e-7 #-2.5e-7 # -0.1 # 惩罚过快的joint 加速度
            dof_vel_arm = -1.e-4
            collision = -5. # 惩罚大腿小腿躯干触地
            # action_smoothness = -0.02
            action_rate = -0.015 # 惩罚action 变化速度
            action_rate_arm = -0.02 # 惩罚action 变化速度
...
class B2Z1ForceRealRobotRoughCfgPPO( LeggedRobotCfgPPO ):
    class algorithm( LeggedRobotCfgPPO.algorithm ):
        entropy_coef = 0.01
    class runner( LeggedRobotCfgPPO.runner ):
        run_name = ''
        experiment_name = 'b2z1_force_realrobot'
