...
from legged_gym.envs.b2.z1_realrobot_config import Z1RealRobotRoughCfg
...
class LeggedRobot_z1_realrobot(BaseTask):
    def __init__(self, cfg: Z1RealRobotRoughCfg, sim_params, physics_engine, sim_device, headless):
    ...
    def compute_observations(self):
        ...
        ee_local_cart = quat_rotate_inverse(self.base_quat, self.ee_pos - arm_base_pos)
    ...
    def get_measured_ee_pos_spherical(self) -> torch.Tensor:
        ...
        ee_idx = self.gym.find_actor_rigid_body_handle(self.envs[0], self.actor_handles[0], "gripperStator")
        ee_position_world = self.rigid_state[:, ee_idx, 0:3].view(self.num_envs, 3) # env.rigid_body_state.shape = (num_envs, num_rigid_bodies, 13) = (1, 24, 13)
        ...
        z_base_pos_world = torch.ones_like(self.base_pos[:, 2].view(self.num_envs, 1))*0.6
        ...
        ee_position_arm[:,0] = ee_position_base[:,0].add_(+0.05)
        ee_position_arm[:,1] = ee_position_base[:,1]
        ee_position_arm[:,2] = ee_position_base[:,2].add_(-0.1)
    ...
    def _resample_ee_goal(self, env_ids, is_init=False):
        ...
        if len(env_ids) > 0:
            ...
            # arm_base_pos = self.base_pos + quat_apply(self.base_yaw_quat, self.arm_base_offset)
            ee_local_cart = quat_rotate_inverse(self.base_quat, self.ee_pos - self.get_ee_goal_spherical_center())
                    # Spherical to cartesian coordinates in the arm base frame 
