...
class LeggedRobot_b2z1_realrobot(BaseTask):
    ...
    def _reward_dof_vel_arm(self):
        # Penalize dof velocities
        return torch.sum(torch.square(self.dof_vel)[:, 12:18], dim=1)
    ...
    def _reward_energy_square_arm(self):
        energy = torch.sum(torch.square(self.torques * self.dof_vel)[:, 12:18], dim=1)
        return energy
    ...
    def _reward_dof_acc_arm(self):
        # Penalize dof accelerations
        return torch.sum(torch.square((self.last_dof_vel - self.dof_vel)[:, 12:18] / self.dt), dim=1)
    ...
    def _reward_action_rate_arm(self):
        # Penalize changes in actions
        return torch.sum(torch.square(self.last_actions - self.actions)[:, 12:18], dim=1)
    ...
    def _reward_dof_pos_limits(self):
        ...
        return torch.sum(out_of_limits[:, :18], dim=1)
