...
class LeggedRobot_b2z1_realrobot(BaseTask):
    def __init__(self, cfg: B2Z1RealRobotRoughCfg, sim_params, physics_engine, sim_device, headless):
        ...
        if self.debug_curve:
            self.arm2_joint_index = 13
            self.arm3_joint_index = 14
            self.arm4_joint_index = 15
            self.arm5_joint_index = 16
            # 初始化绘图
            plt.ion()  # 打开交互模式
            fig, ((self.ax1, self.ax4), (self.ax2, self.ax5)) = plt.subplots(2, 2)
            self.t_data, self.arm2_action, self.arm2_state, self.arm3_action, self.arm3_state = [], [], [], [], []
            self.arm4_action, self.arm4_state, self.arm5_action, self.arm5_state = [], [], [], []
            self.arm2_action_line, = self.ax1.plot([], [], 'b-', label="arm2_action")       
            self.arm2_state_line, = self.ax1.plot([], [], 'r-', label="arm2_state")
            # self.FL_ref_line, = self.ax1.plot([], [], 'y-', label="FL_ref")
            self.arm3_action_line, = self.ax2.plot([], [], 'b-', label="arm3_action")
            self.arm3_state_line, = self.ax2.plot([], [], 'r-', label="arm3_state")
            # self.FR_ref_line, = self.ax2.plot([], [], 'y-', label="FR_ref")
            self.arm4_action_line, = self.ax4.plot([], [], 'b-', label="arm4_action")       
            self.arm4_state_line, = self.ax4.plot([], [], 'r-', label="arm4_state")
            # self.RL_ref_line, = self.ax4.plot([], [], 'y-', label="RL_ref")
            self.arm5_action_line, = self.ax5.plot([], [], 'b-', label="arm5_action")
            self.arm5_state_line, = self.ax5.plot([], [], 'r-', label="arm5_state")
    ...
    def step(self, actions):
        ...
        #         self.actions[:, 17] = 0.
        #     self._compute_position(self.actions)
        #     self.gym.set_dof_position_target_tensor(self.sim, gymtorch.unwrap_tensor(self.joint_pos_target))
            ...
            #         self.env_nb = torch.arange(self.num_envs, device=self.device)
            #         self.ee_init_pos_world = self.rigid_body_state[:, self.gripper_stator_index, 0:3].view(self.num_envs, 3).clone()
        ...
        for _ in range(self.cfg.control.decimation):
            self._compute_position(self.actions)
            all_pos_targets = torch.zeros_like(self.dof_pos)
            all_pos_targets[:, -(6 + self.cfg.env.num_gripper_joints):-self.cfg.env.num_gripper_joints] = self.joint_pos_target[:, -(6 + self.cfg.env.num_gripper_joints):-self.cfg.env.num_gripper_joints]
            self.torques = self._compute_torques(self.actions)
            b2_torques = self.torques.clone()
            b2_torques[:, -7:-1] = 0.
            self.gym.set_dof_position_target_tensor(self.sim, gymtorch.unwrap_tensor(all_pos_targets))
            self.gym.set_dof_actuation_force_tensor(self.sim, gymtorch.unwrap_tensor(b2_torques))
    ...
    def post_physics_step(self):
        ...
        self.last_last_joint_pos_target[:] = torch.clone(self.last_joint_pos_target[:])
    ...
    def reset_idx(self, env_ids):
        ...
        self._resample_ee_goal(env_ids, is_init=False)
    ...
    def compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            ...
            if name == "feet_contact_forces":
                self.save_feet_contact_forces = rew
            ...
            elif self.cfg.rewards.only_negative_rewards_ji22_style:
            ...
            self.rew_buf[:] = self.rew_buf * self.cfg.rewards.total_reward_scale
    ...
    def get_ee_force(self, env_ids = None):
        ...
        forces_global = self.forces[:, self.gripper_stator_index, 0:3]
        ...
        return forces_local.view(self.num_envs, 3)

    def compute_observations(self):
        """ Computes observations
        """
        # arm_base_pos = self.base_pos + quat_apply(self.base_yaw_quat, self.arm_base_offset)
        dog_base = self.get_ee_goal_spherical_center()
        ee_goal_local_cart = quat_rotate_inverse(self.base_yaw_quat, self.curr_ee_goal_cart_world - dog_base)
        ee_pos_local_cart = quat_rotate_inverse(self.base_yaw_quat, self.ee_pos - dog_base)

        self.privileged_obs_buf = torch.cat((  self.base_lin_vel * self.obs_scales.lin_vel,
                                    self.projected_gravity,ee_pos_local_cart,self.get_ee_force() * self.obs_scales.force,ee_goal_local_cart
                                    ),dim=-1)
        # print("pos_state:", self.dof_pos[0])
        obs_buf = torch.cat(( self.get_body_orientation(),  # dim 2
                                    ...
                                    self.force_command * self.obs_scales.force  # dim 3
                                    ),dim=-1)
    ...
    def _draw_curve(self):
        if self.debug_curve:
            ...
            # 设置绘图范围
            self.ax1.set_ylim(self.default_dof_pos_wo_gripper[0, self.arm2_joint_index].item()-1.5, self.default_dof_pos_wo_gripper[0, self.arm2_joint_index].item()+1.5)
            self.ax1.set_title(f"arm2_joint_{self.arm2_joint_index}")
            self.ax1.set_xlabel("time / s")
            self.ax1.legend(loc="upper right")
            self.ax2.set_ylim(self.default_dof_pos_wo_gripper[0, self.arm3_joint_index].item()-1.5, self.default_dof_pos_wo_gripper[0, self.arm3_joint_index].item()+1.5)
            self.ax2.set_title(f"arm3_joint_{self.arm3_joint_index}")
            ...
            self.ax4.set_ylim(self.default_dof_pos_wo_gripper[0, self.arm4_joint_index].item()-1.5, self.default_dof_pos_wo_gripper[0, self.arm4_joint_index].item()+1.5)
            self.ax4.set_title(f"arm4_joint_{self.arm4_joint_index}")
            self.ax4.set_xlabel("time / s")
            self.ax4.legend(loc="upper right")
            self.ax5.set_ylim(self.default_dof_pos_wo_gripper[0, self.arm5_joint_index].item()-1.5, self.default_dof_pos_wo_gripper[0, self.arm5_joint_index].item()+1.5)
            self.ax5.set_title(f"arm5_joint_{self.arm5_joint_index}")
            ...
            self.arm2_action.append(self.actions[0, self.arm2_joint_index].item() * self.cfg.control.action_scale + self.default_dof_pos_wo_gripper[0, self.arm2_joint_index].item())
            self.arm2_state.append(self.dof_pos[0, self.arm2_joint_index].item())
            # self.FL_ref.append(self.ref_dof_pos[0, self.FL_joint_index].item())
            self.arm3_action.append(self.actions[0, self.arm3_joint_index].item() * self.cfg.control.action_scale + self.default_dof_pos_wo_gripper[0, self.arm3_joint_index].item())
            self.arm3_state.append(self.dof_pos[0, self.arm3_joint_index].item())
            # self.FR_ref.append(self.ref_dof_pos[0, self.FR_joint_index].item())

            # stand_mask = self._get_gait_phase()
            self.arm4_action.append(self.actions[0, self.arm4_joint_index].item() * self.cfg.control.action_scale + self.default_dof_pos_wo_gripper[0, self.arm4_joint_index].item())
            self.arm4_state.append(self.dof_pos[0, self.arm4_joint_index].item())
            # self.RL_ref.append(self.ref_dof_pos[0, self.RL_joint_index].item())
            self.arm5_action.append(self.actions[0, self.arm5_joint_index].item() * self.cfg.control.action_scale + self.default_dof_pos_wo_gripper[0, self.arm5_joint_index].item())
            self.arm5_state.append(self.dof_pos[0, self.arm5_joint_index].item())
            # self.RR_ref.append(self.ref_dof_pos[0, self.RR_joint_index].item())
            
            if len(self.t_data) > 100:
                self.t_data.pop(0)
                self.arm2_action.pop(0)
                self.arm2_state.pop(0)
                # self.FL_ref.pop(0)
                self.arm3_action.pop(0)
                self.arm3_state.pop(0)
                # self.FR_ref.pop(0)
                self.arm4_action.pop(0)
                self.arm4_state.pop(0)
                # self.RL_ref.pop(0)
                self.arm5_action.pop(0)
                self.arm5_state.pop(0)
                # self.RR_ref.pop(0)
            # 更新绘图
            self.arm2_action_line.set_data(self.t_data, self.arm2_action)
            self.arm2_state_line.set_data(self.t_data, self.arm2_state)
            # self.FL_ref_line.set_data(self.t_data, self.FL_ref)
            self.arm3_action_line.set_data(self.t_data, self.arm3_action)
            self.arm3_state_line.set_data(self.t_data, self.arm3_state)
            # self.FR_ref_line.set_data(self.t_data, self.FR_ref)

            self.arm4_action_line.set_data(self.t_data, self.arm4_action)
            self.arm4_state_line.set_data(self.t_data, self.arm4_state)
            # self.RL_ref_line.set_data(self.t_data, self.RL_ref)
            self.arm5_action_line.set_data(self.t_data, self.arm5_action)
            self.arm5_state_line.set_data(self.t_data, self.arm5_state)
    ...
    def _draw_ee_goal_curr(self):
        ...
        # ee_pose = self.ee_position_cmd_world

        sphere_geom_1 = gymutil.WireframeSphereGeometry(0.05, 4, 4, None, color=(1, 0, 1))
        ...
        for i in range(self.num_envs):
            sphere_pose = gymapi.Transform(gymapi.Vec3(self.ee_goal_force_offset[i, 0], self.ee_goal_force_offset[i, 1], self.ee_goal_force_offset[i, 2]), r=None)
    ...
    def _draw_ee_force(self):
        ...
        forces_cmd = self.current_Fxyz_cmd / 100
    ...
    def _compute_position(self,actions):
        ...
        if self.cfg.control.control_type == "P":
            ...
            self.joint_pos_target = actions_scaled * self.dt / self.cfg.control.decimation
    ...
    def _compute_torques(self, actions):
        ...
        # torques = self.p_gains*(actions_scaled + self.default_dof_pos[:, :18] - self.dof_pos[:,:18]) - self.d_gains*self.dof_vel[:,:18]
        # default_torques[:, -6:] = 0
    ...
    def _reset_dofs(self, env_ids):
        ...
        self.dof_pos[env_ids, :12] = self.default_dof_pos[:12]* torch_rand_float(0.5, 1.5, (len(env_ids), self.num_dof), device=self.device)
    ...
    def _push_gripper(self, env_ids_all, cfg):
        ...
        if self.episode_length_buf[self.selected_env_ids == 1].nelement() > 0:
            ...
            if env_ids_apply_push_step1.nelement() > 0:
                ...
                # World frame
                self.force_command[env_ids_apply_push_step1, INDEX_EE_FORCE_X] = self.current_Fxyz_cmd[env_ids_apply_push_step1, 0] #torch.norm(self.current_Fxyz_cmd[env_ids_apply_push_step1, :2], dim=1)
                self.force_command[env_ids_apply_push_step1, INDEX_EE_FORCE_Y] = self.current_Fxyz_cmd[env_ids_apply_push_step1, 1] #torch.atan2(self.current_Fxyz_cmd[env_ids_apply_push_step1, 1], self.current_Fxyz_cmd[env_ids_apply_push_step1, 0])
                self.force_command[env_ids_apply_push_step1, INDEX_EE_FORCE_Z] = self.current_Fxyz_cmd[env_ids_apply_push_step1, 2]
            ...
            if env_ids_apply_push_step2.nelement() > 0:
                ...
                # World frame
                self.force_command[env_ids_apply_push_step2, INDEX_EE_FORCE_X] = self.current_Fxyz_cmd[env_ids_apply_push_step2, 0] #torch.norm(self.current_Fxyz_cmd[env_ids_apply_push_step2, :2], dim=1)
                self.force_command[env_ids_apply_push_step2, INDEX_EE_FORCE_Y] = self.current_Fxyz_cmd[env_ids_apply_push_step2, 1] #torch.atan2(self.current_Fxyz_cmd[env_ids_apply_push_step2, 1], self.current_Fxyz_cmd[env_ids_apply_push_step2, 0])
                self.force_command[env_ids_apply_push_step2, INDEX_EE_FORCE_Z] = self.current_Fxyz_cmd[env_ids_apply_push_step2, 2]
            ...
            if env_ids_to_reset.nelement() > 0:
                ...
                self.force_command[env_ids_to_reset, INDEX_EE_FORCE_X] = 0.0
                self.force_command[env_ids_to_reset, INDEX_EE_FORCE_Y] = 0.0

                self.force_command[env_ids_to_reset, INDEX_EE_FORCE_Z] = 0.0
        ...
        self.force_command[self.freed_envs, INDEX_EE_FORCE_X] = 0.0
        self.force_command[self.freed_envs, INDEX_EE_FORCE_Y] = 0.0
        self.force_command[self.freed_envs, INDEX_EE_FORCE_Z] = 0.0

        # # Commands in spherical coordinates in the arm base frame 
        # radius_cmd = self.ee_goal_sphere[:, 0].view(self.num_envs, 1) 
        # pitch_cmd = self.ee_goal_sphere[:, 1].view(self.num_envs, 1) 
        # yaw_cmd = self.ee_goal_sphere[:, 2].view(self.num_envs, 1) 
        ...
        self.ee_position_cmd_world = self.curr_ee_goal_cart_world
        ...
        self.ee_pos_world = self.rigid_body_state.view(self.num_envs, -1, 13)[:,self.gripper_idx,0:3].view(self.num_envs,3)
        ...
        # self.forces[:, self.gripper_stator_index, :3] = torch.clamp(((self.ee_position_cmd_world - self.ee_pos_world) * self.gripper_force_kps), self.cfg.domain_rand.max_push_force_xyz_gripper_freed[0], self.cfg.domain_rand.max_push_force_xyz_gripper_freed[1])
        # self.forces[:, self.gripper_stator_index, :3] = self.force_command * -1
        ...
        if self.global_steps < 8000 * 24 :
            ...
            # self.ee_goal_force_offset = self.curr_ee_goal_cart_world
            self.ee_goal_force_offset[:,:] = self.curr_ee_goal_cart_world + self.current_Fxyz_cmd / self.gripper_force_kps
            self.force_offset = self.forces[:, self.gripper_stator_index, :3] - self.current_Fxyz_cmd
            self.vel_offset = quat_rotate_inverse(self.base_quat, self.force_offset / self.gripper_force_kds)

            self.commands_offset[:, :2] = self.commands[:, :2]  + self.vel_offset[:, :2]
        ...
        force_magn_target = torch.norm(self.force_command[:, INDEX_EE_FORCE_X:INDEX_EE_FORCE_Y+1], dim=1).view(self.num_envs) 
        ...
        force_magn_cmd = (self.force_command[:, INDEX_EE_FORCE_Z]).view(self.num_envs, 1)
    ...
    def _init_buffers(self):
        ...
        self.arm_base_offset = torch.tensor([0.2, 0., 0.2], device=self.device, dtype=torch.float).repeat(self.num_envs, 1)
        ...
        self.ee_goal_force_offset = torch.zeros(self.num_envs, 3, device=self.device)
        self.force_offset = torch.zeros(self.num_envs, 3, device=self.device)
        self.vel_offset = torch.zeros(self.num_envs, 3, device=self.device)
        self.commands_offset = torch.zeros(self.num_envs, 3, device=self.device)
        ...
        #arm force control push gripper
        self.rigid_body_state = gymtorch.wrap_tensor(rigid_body_state)[:self.num_envs*self.num_bodies,:].view(self.num_envs,self.num_bodies,13)[:,0:self.num_bodies, :]
        self.gripper_velocity = self.rigid_body_state.view(self.num_envs, self.num_bodies, 13)[:, self.gripper_stator_index, 7:10]
        self.force_command = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _create_envs(self):
        ...
        self.gripper_stator_index = [index for index, body_name in enumerate(body_names) if body_name == "ee_gripper_link"][0]
        ...
        self.gripper_idx = self.body_names_to_idx["ee_gripper_link"]
        ...
        for i in range(self.num_envs):
            ...
            dof_props_asset['damping'][12:].fill(arm_kd)
            dof_props_asset['stiffness'][13] = 128.0
            dof_props_asset['damping'][13] = 3.0
    ...
    def _reward_tracking_ee_world(self):
        ee_pos_error = torch.sum(torch.abs(self.ee_pos - self.ee_goal_force_offset), dim=1)
    ...
    def _reward_dof_acc_arm(self):
        # Penalize dof accelerations
        return torch.sum(torch.square((self.last_dof_vel - self.dof_vel)[:, -6:] / self.dt), dim=1)
    ...
    def _reward_tracking_lin_vel(self):
        ...
        lin_vel_error = torch.sum(torch.square(self.commands_offset[:, :2] - self.base_lin_vel[:, :2]), dim=1)
    ...
    def _reward_action_smoothness_1_leg(self):
        ...
        diff = diff * (self.last_actions[:, :12] != 0)
    ...
    def _reward_action_smoothness_2_leg(self):
        ...
        diff = diff * (self.last_actions[:, :12] != 0)
        diff = diff * (self.last_last_actions[:, :12] != 0)
        return torch.sum(diff, dim=1)
    
    def _reward_action_smoothness_1_arm(self):
        diff = torch.square(self.joint_pos_target[:, 12:17] - self.last_joint_pos_target[:, 12:17])
        diff = diff * (self.last_actions[:, 12:17] != 0)
        return torch.sum(diff, dim=1)
    
    def _reward_action_smoothness_2_arm(self):
        diff = torch.square(self.joint_pos_target[:, 12:17] - 2 * self.last_joint_pos_target[:, 12:17] + self.last_last_joint_pos_target[:, 12:17])
        diff = diff * (self.last_actions[:, 12:17] != 0)
        diff = diff * (self.last_last_actions[:, 12:17] != 0)
        return torch.sum(diff, dim=1)
    ...
    def _reward_ee_force_x(self):
        ...
        force_magn_cmd = (self.force_command[:, INDEX_EE_FORCE_X]).view(self.num_envs, 1)
    ...
    def _reward_ee_force_y(self):
        ...
        force_magn_cmd = (self.force_command[:, INDEX_EE_FORCE_Y]).view(self.num_envs, 1)
        force_magn_error = torch.abs(force_magn_meas - force_magn_cmd).view(self.num_envs)
