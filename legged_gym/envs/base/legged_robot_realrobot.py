...
class LeggedRobot_realrobot(BaseTask):
    ...
    def compute_observations(self):
        ...
        obs_buf = torch.cat(( self.get_body_orientation(),  # dim 2
                                    ...
                                    (self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos, # dim 12
                                    self.dof_vel * self.obs_scales.dof_vel, # dim 12
                                    self.actions, # dim 12
    ...
    def get_walking_cmd_mask(self, env_ids=None, return_all=False):
        ...
        walking_mask1 = torch.norm(self.commands[:, :2], dim=1) > 0.1
        walking_mask2 = torch.abs(self.commands[:, 2]) > 0.1
        
        walking_mask = walking_mask1 | walking_mask2
        return walking_mask
    ...
    def _reward_feet_height(self):
        ...
        rew = torch.clamp(torch.max(feet_height, dim=-1)[0] - 0.20, max=0)
        cmd_stop_flag = ~self.get_walking_cmd_mask()
        rew[cmd_stop_flag] = 0
        return rew
    ...
    def _reward_feet_height_high(self):
        ...
        rew = torch.clamp(torch.max(feet_height, dim=-1)[0] - 0.25, min=0)
        cmd_stop_flag = ~self.get_walking_cmd_mask()
        rew[cmd_stop_flag] = 0
        return rew
