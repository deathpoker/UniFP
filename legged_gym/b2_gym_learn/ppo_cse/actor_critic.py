...
class AC_Args(PrefixProto, cli=False):
    ...
    adaptation_labels = ["base_velocity_loss", "gripper_pos_loss", "force_loss"] #, "force_loss"]
    adaptation_dims = [3, 3, 3] #, 3]
    adaptation_weights = [1.0, 1.0, 1.0] #, 1]
...
class ActorCritic(nn.Module):
    ...
    def __init__(self, num_obs,
        ...
        self.num_estimated_obs = sum(self.adaptation_dims)
        ...
        for l in range(len(AC_Args.adaptation_module_branch_hidden_dims)):
            if l == len(AC_Args.adaptation_module_branch_hidden_dims) - 1:
                adaptation_module_layers.append(
                    nn.Linear(AC_Args.adaptation_module_branch_hidden_dims[l], self.num_estimated_obs))
            else:
                adaptation_module_layers.append(
        ...
        # Policy
        actor_layers = []
        actor_layers.append(nn.Linear(self.num_estimated_obs + self.num_obs_history, AC_Args.actor_hidden_dims[0]))
